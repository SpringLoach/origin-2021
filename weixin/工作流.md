...
图片 | wx.saveImageToPhotosAlbum | 保存图片到系统相册，需用户授权
图片 | wx.chooseImage | 从本地相册选择图片或使用相机拍照
位置 | wx.stopLocationUpdate | 关闭监听实时位置变化，前后台都停止消息接收
位置 | wx.startLocationUpdateBackground | 开启小程序进入前后台时均接收位置消息，需用户授权，[配置](https://developers.weixin.qq.com/miniprogram/dev/reference/configuration/app.html#permission)
位置 | wx.startLocationUpdate | 开启小程序进入前台时接收位置消息，需用户授权
位置 | wx.openLocation | 使用微信内置地图**查看位置**
位置 | wx.chooseLocation | 打开地图**选择位置**
登录 | wx.login | 调用接口获取登录凭证（code）
用户信息 | wx.getUserProfile | 获取用户信息，包括头像、昵称、所在城市等，需用户授权
授权 | wx.authorize | 提前向用户发起授权请求
设置 | wx.openSetting | 调起客户端小程序设置界面，返回小程序已经向用户请求过的权限
设置 | wx.getSetting | 获取用户的当前设置
...
getCurrentPages | 获取当前页面栈。数组中最后一个元素为当前页面
